...
- セッション管理を用いた安全なログイン・ログアウト
- ダッシュボード: 全商品数、低在庫数、在庫切れ数、総売上の一覧表示
- **商品管理**: 追加・編集、商品名またはSKUによる検索、在庫状態によるフィルタリング
- **売上管理**: 売上登録時に在庫を自動減算、過剰販売防止チェック付き
- 検索・日付範囲フィルタリング対応の売上履歴
- 全入力項目にサーバーサイドバリデーションを実装
- 在庫ステータスはリアルタイムで動的に算出
- **バイリンガルUI（英/日）**: Flaskセッションで言語設定を保持。全UIテキスト、フラッシュメッセージ、在庫ステータスラベルをアクティブな言語で表示
- **ロールベースアクセス制御**: `admin_required`デコレータで商品追加・編集・売上登録ルートを管理者のみに制限。一般アカウントは読み取り専用、管理者向けUIボタンは非表示
...
| SSH -> 最新コード取得 -> 依存パッケージ導入 -> サービス再起動      |
+------------------------------+---------------------------------+
                               |
                               v
+----------------------------------------------------------------+
|     EC2本番サーバー  aman-inventory-prod（東京リージョン）       |
|                                                                |
|ブラウザ -> Nginx（ポート80）-> Gunicorn -> Flask（ポート5000）   |
|                                                                |
|       IAMロール認証（クレデンシャルのハードコードなし）            |
+------------+---------------------------------------------------+
           |                                 |
           v                                 v
+--------------------+   +---------------------------------------+
|   RDS MySQL 8.4    |   |            AWS連携サービス             |
|  在庫データベース    |   |                                       |
| （東京リージョン）   |   |  SES      -> 低在庫メール通知           |
+--------------------+   |  Bedrock  -> AI発注数予測              |
                         |             （毎日深夜2時 cronジョブ）  |
                         |  CloudWatch-> CPU監視・SNS通知         |
                         +---------------------------------------+
```
...
| RDSエンジン | MySQL Community 8.4.8、db.t4g.micro、ap-northeast-1（非公開; EC2セキュリティグループからのみアクセス可） |
...
- AWSクレデンシャルのハードコードなし; EC2はIAMロールで認証
- `.env`はgitignore済みでGitHubにプッシュされない
- パスワードは`werkzeug.security`のPBKDF2でハッシュ化
- 全ルートに`@login_required`を適用; 書き込み操作には追加で`@admin_required`を適用
- ロールベースアクセス制御: 一般アカウント・デモアカウントは読み取り専用。管理者UIはテンプレートレベルで非表示にし、ルートレベルでもブロック
- ポート5000は外部に公開せず; 全トラフィックはNginx（ポート80）経由
- RDSセキュリティグループはMySQL（ポート3306）をEC2セキュリティグループからのみ許可し、外部からの直接アクセスは不可
- SSH接続は鍵認証のみ（`sshd_config`でパスワード認証を無効化）; ポート22は動的IPを使用するGitHub ActionsランナーのCI/CD対応のため`0.0.0.0/0`に開放
...
本プロジェクトはポートフォリオおよび学習目的で作成しています。
