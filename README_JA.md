...
| サーバー | Ubuntu 24 EC2 · Nginx リバースプロキシ · Gunicorn · systemd |
...
| i18n | 日英バイリンガル — Flask セッションによる言語切替 |
...
- **Amazon Bedrock（Claude Haiku 4.5）** — 毎日午前 2 時（EC2 時刻）の cron ジョブが低在庫商品の過去 30 日分の売上実績を取得し、Bedrock を呼び出して `recommended_restock_qty`（推奨補充数量）と reasoning（日本語・英語）を生成。結果は `predictions` テーブルに保存され、ダッシュボードのアクティブ言語で表示
---
...
|                                                                |
|  ブラウザ -> Nginx (80番) -> Gunicorn -> Flask (5000番 内部)   |
|                                                                |
...
1. 社員が商品と数量を選択
2. システムが在庫数を検証
3. 在庫が十分な場合 → 売上レコードを挿入し `stock_quantity` をアトミックに減算
4. 在庫が不足する場合 → エラーを返し、データは変更しない
---
...
│   └── predict.py             # Bedrock AI 需要予測（日次 cron・バイリンガル）
├── static/
│   ├── css/style.css
│   └── js/
│       ├── app.js
│       └── i18n.js            # 日英切替（/set-lang/ ルートを呼び出し）
├── templates/
│   ├── base.html              # AdminLTE シェル（サイドバー + ナビバー）
│   ├── dashboard.html         # AI 補充推奨セクションを含むダッシュボード（バイリンガル）
│   ├── login.html
...
- [x] Nginx リバースプロキシ
- [x] Gunicorn 本番 WSGI サーバー（3 ワーカー）
